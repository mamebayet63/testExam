...
if ($pdo !== null) {
    try {
        // Requête pour récupérer les abonnés de la page courante
        $query = $pdo->prepare("SELECT * FROM livre LIMIT :offset, :limit");
        ...
        $totalQuery = $pdo->query("SELECT COUNT(*) FROM abonne");
...
<body class="">
    <div class="container">
                ...
                <div class="col-10 mt-4">
                    <table class="table table-striped">
                        ...
                        <?php foreach ($abonnes as $k => $abonne) : ?>
                            <tr>
                                <td><img src="<?= $abonne['images'] ?>" class="monImage" alt=""></td>
                                <td><?= $abonne['titre'] ?></td>
                                <td><?= $abonne['auteur'] ?></td>
                                <td><?= $abonne['isbn'] ?></td>
                                <td>
            ...
            <nav aria-label="Page navigation">
                <ul class="pagination justify-content-center mt-4">
                    <?php if ($pageCourante > 1): ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?= $pageCourante - 1 ?>" aria-label="Previous">
                    ...
                    <?php if ($pageCourante < $nombrePages): ?>
                        <li class="page-item">
                            <a class="page-link" href="?page=<?= $pageCourante + 1 ?>" aria-label="Next">
        ...
        <?php else : ?>
            <p class="mt-3">Aucune abonné enregistré pour le moment...</p>
        <?php endif; ?>
    </div>
